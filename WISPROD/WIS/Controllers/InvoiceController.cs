...
namespace WIS.Controllers
{
    public class InvoiceController : Controller
    {
        ...
        public ActionResult Invoices()
        {
            ...
            var model = service.GetInvoiceListwithExternalDtls();
            ...
            {
                //write a join for externaldetaild and Invoice table
               item.CreatedUser = GetUserFullname(item.CreatedUser);                             
            }
            return View(model);
        }
        public ActionResult Estimates()
        {
            ...
            {
                item.CreatedUser = GetUserFullname(item.CreatedUser);
                if (item.ExternalClientID != null)
                {
                    var FirstName = service.GetExternalDtlsById(item.ExternalClientID.Value).FirstName;
                    item.ExernalClientName = service.GetExternalDtlsById(item.ExternalClientID.Value).FirstName;
                    var LastName = service.GetExternalDtlsById(item.ExternalClientID.Value).LastName;
                    ViewBag.EXternalClientName = FirstName + " " + LastName;
                    var externaldetails = service.GetExternalDtlsById(item.ExternalClientID.Value);
                    ViewBag.Addres = externaldetails.Address;
                    ViewBag.city = externaldetails.City;
                    ViewBag.sts = externaldetails.State;
                    ViewBag.zp = externaldetails.Zip;
                    item.Employee = ViewBag.EmployeeName;
                }
            }
            return View(model);
        }
        ...
        public ActionResult PopupAddInvoiceItem(int Location)
        {
            ...
            var GLlst = new List<SelectListItem>();
            GLlst.Add(new SelectListItem { Text = "CW29200.000.000070", Value = "CW29200.000.000070" });
            GLlst.Add(new SelectListItem { Text = "BM29200.000.000050", Value = "BM29200.000.000050" });
            ViewBag.Gllists = GLlst;
            ViewBag.Category = service.GetItemCategories().Select(p => new SelectListItem { Text = p.ItemCategoryDescription, Value = p.ItemCategoryID.ToString() }).ToList();
        ...
        [HttpPost]
        public ActionResult AddInvoiceItems(int Id, int qty, decimal price, string CGCode,string InvCode,string desc, int cat, string catText,int locaton)
        {
            ...
            lst.InvoiceDetails.Add(new InvoiceDetailModel { InvoiceDetailID = lst.InvoiceDetails.Count, ItemInventoryDescription = desc, ItemInventoryID = Id, InvoiceDetailQuantity = qty, ItemInventoryUnitCost = price, InvoiceDetailLineItemTotal = qty * price, InvoiceDetailCostCodeGL = CGCode,InvoiceDetailCostCodeNonLabor=InvCode, Categories = catText, ItemCategoryID = cat });
        ...
        [HttpPost]
        public ActionResult AddEstimateItems(int Id, int qty, decimal price, string CGCode,string InvCode, string desc, int cat, string catText, int locaton)
        {
            ...
            lst.EstimateDetails.Add(new EstimateDetailModel { EstimateDetailID = lst.EstimateDetails.Count, EstimateDescription = desc, ItemInventoryID = Id, EstimateDetailQuantity = qty, ItemInventoryCost = price, EstimateDetailLineItemTotal = qty * price, EstimateDetailCostCodeGL = CGCode,EstimateDetailCostCodeNonLabor=InvCode, Categories = catText, ItemCategoryID = cat });
        ...
        [HttpPost]
        public JsonResult saveCategoriesValtoDb(string CatVal, string ItmIncludes)
        {
            var service = new WISService();
            var model = service.AddCategoryValue(CatVal,ItmIncludes);
            return Json(model, JsonRequestBehavior.AllowGet);           
        }
        [HttpPost]
        public JsonResult saveCategoriesDescptoDb(string CatVal,int Id)
        {
            var service = new WISService();
            var model = service.EditCategoryDesc(CatVal, Id);
            return Json(model, JsonRequestBehavior.AllowGet);           
        }
        public ActionResult AddInvoiceItems1(int Id, int qty, decimal price, string CGCode, string InvCode, string itm1, int InvoiceId)
        {
            var lst = HttpContext.Session["Invoice"] as InvoiceModel ?? new InvoiceModel();
            if (lst.InvoiceDetails == null)
            {
            ...
            lst.InvoiceDetails.Add(new InvoiceDetailModel { InvoiceDetailID = lst.InvoiceDetails.Count,  InvoiceDetailQuantity = qty, ItemInventoryUnitCost = price, InvoiceDetailLineItemTotal = qty * price, InvoiceDetailCostCodeGL = CGCode,InvoiceDetailCostCodeNonLabor=InvCode, NonInventoryItem = itm1 });
            lst.InvoiceTotal = GetTotal(lst);           
            ViewBag.Total = lst.InvoiceTotal;
            HttpContext.Session["Invoice"] = lst;
            return View("_InvoiceDetails", lst.InvoiceDetails);
        }
        public ActionResult AddEstimateItems1(int Id, int qty, decimal price, string CGCode, string InvCode, string itm1, int EstimateId)
        {
            var lst = HttpContext.Session["Estimates"] as EstimateModel ?? new EstimateModel();
            if (lst.EstimateDetails == null)
            {
            ...
            lst.EstimateDetails.Add(new EstimateDetailModel { EstimateDetailID = lst.EstimateDetails.Count, EstimateDetailQuantity = qty, ItemInventoryCost = price, EstimateDetailLineItemTotal = qty * price, EstimateDetailCostCodeGL = CGCode,EstimateDetailCostCodeNonLabor=InvCode, NonInventoryItem = itm1 });
            lst.Total = GetEstimateTotal(lst);
            ViewBag.Total = lst.Total;
            HttpContext.Session["Estimates"] = lst;
            return View("_estimateDetails", lst.EstimateDetails);
        }
        public ActionResult EditAddInvoiceItems(int Id, int qty, decimal price, string CGCode,string InvCode, string desc, int cat, string catText,int InvoiceId)
        {
            var service = new WISService();
            bool sucess = service.SaveInvoiceItem(InvoiceId, new InvoiceDetailModel {  ItemInventoryDescription = desc, ItemInventoryID = Id, InvoiceDetailQuantity = qty, ItemInventoryUnitCost = price, InvoiceDetailLineItemTotal = qty * price, InvoiceDetailCostCodeGL = CGCode,InvoiceDetailCostCodeNonLabor=InvCode, Categories = catText, ItemCategoryID = cat });
            var lst = service.GetInvoiceDetails(InvoiceId);            
            lst.InvoiceTotal = GetTotal(lst);
            ViewBag.Total = lst.InvoiceTotal;          
            return View("_InvoiceDetails", lst.InvoiceDetails);
        }
        public ActionResult EditAddInvoiceItems1(int Id, int qty, decimal price, string CGCode,string InvCode, string itm1, int InvoiceId)
        {
            var service = new WISService();
            bool sucess = service.SaveInvoiceItem(InvoiceId, new InvoiceDetailModel { ItemInventoryID = Id==0 ? (int?)null :Id, InvoiceDetailQuantity = qty, ItemInventoryUnitCost = price, InvoiceDetailLineItemTotal = qty * price, InvoiceDetailCostCodeGL = CGCode, InvoiceDetailCostCodeNonLabor=InvCode,NonInventoryItem = itm1 });
            var lst = service.GetInvoiceDetails(InvoiceId);          
            lst.InvoiceTotal = GetTotal(lst);
            ViewBag.Total = lst.InvoiceTotal;
            return View("_InvoiceDetails", lst.InvoiceDetails);
        }
        public ActionResult EditAddEstimateItems(int Id, int qty, decimal price, string CGCode, string InvCode, string desc, int cat, string catText, int EstimateId)
        {
            var service = new WISService();
            bool sucess = service.SaveEstimateItem(EstimateId, new EstimateDetailModel { EstimateDescription = desc, ItemInventoryID = Id, EstimateDetailQuantity = qty, ItemInventoryCost = price, EstimateDetailLineItemTotal = qty * price, EstimateDetailCostCodeGL = CGCode,EstimateDetailCostCodeNonLabor=InvCode, Categories = catText, ItemCategoryID = cat });
            var lst = service.GetEstimateDetails(EstimateId);
            lst.Total = GetEstimateTotal(lst);
            ViewBag.Total = lst.Total;
            return View("_estimateDetails", lst.EstimateDetails);
        }
        public ActionResult EditAddEstimateItems1(int Id, int qty, decimal price, string CGCode, string InvCode, string itm1, int EstimateId)
        {
            var service = new WISService();
            bool sucess = service.SaveEstimateItem(EstimateId, new EstimateDetailModel { ItemInventoryID = Id == 0 ? (int?)null : Id, EstimateDetailQuantity = qty, ItemInventoryCost = price, EstimateDetailLineItemTotal = qty * price, EstimateDetailCostCodeGL = CGCode,EstimateDetailCostCodeNonLabor=InvCode, NonInventoryItem = itm1 });
            var lst = service.GetEstimateDetails(EstimateId);
            lst.Total = GetEstimateTotal(lst);
            ViewBag.Total = lst.Total;
            return View("_estimateDetails", lst.EstimateDetails);
        ...
        private EstimateResult SaveEstimate(EstimateModel model)
        {
            var service = new WISService();
            if (string.IsNullOrEmpty(model.InvoiceJobNumber))
            {
        ...
        public ActionResult SaveEditInvoicedtls(InvoiceDetailModel model)
        {
            try
            {
                ...
                {
                    var service = new WISService();
                    if (model.ItemCategoryID!=23 && model.NonInventoryItem==null) {
                        model.ItemInventoryDescription = service.GetInventoryDataById(model.ItemInventoryID.Value).ItemInventoryDescription;
                        //model.ItemInventoryDescription = service.GetCategoryById(model.ItemCategoryID.Value).ItemInventoryDescription;
                    }
        ...
        public ActionResult GetInvoiceSummary(int InvoiceId)
        {
            ...
            //Emodel.InvoiceTotal = GetTotal(Emodel);
            if (Emodel.ExternalClientID != null)
            {
                var externaldetails = service.GetExternalDtlsById(Emodel.ExternalClientID.Value);
                var FirstName = externaldetails.FirstName;
                var LastName = externaldetails.LastName;
                ViewBag.Addres =externaldetails.Address;
                ViewBag.city = externaldetails.City;
                ViewBag.sts = externaldetails.State;
                ViewBag.zp = externaldetails.Zip;
        ...
        public ActionResult GetEstimateSummary(int Estimate)
        {
            ...
            if (Emodel.employID != null && Emodel.employID!=0)
            {
                var FirstName = service.GetEmployeeDtlsById(Emodel.employID).FirstName;
                var LastName = service.GetEmployeeDtlsById(Emodel.employID).LastName;
        ...
        public ActionResult pickupTicketSummaryInv(int InvoiceId)
        {
            var service = new WISService();
            var Emodel = service.GetInvoiceDetails(InvoiceId);
            ViewBag.id = Emodel.InvoiceID;
            ...
            HttpContext.Session["Invoice"] = null;
            return View("PickupTicketSummaryInv", Emodel);
        }
        ...
        public ActionResult PrintSummaryInvoice(int invoiceId)
        {
            ...
            if (Emodel.ExternalClientID != null)
            {
                var FirstName = service.GetExternalDtlsById(Emodel.ExternalClientID.Value).FirstName;
                var LastName = service.GetExternalDtlsById(Emodel.ExternalClientID.Value).LastName;
                ViewBag.EXternalClientName = FirstName + " " + LastName;
                var externaldetails = service.GetExternalDtlsById(Emodel.ExternalClientID.Value);               
                ViewBag.Addres = externaldetails.Address;
                ViewBag.city = externaldetails.City;
        ...
        public ActionResult GetItemInventoryListForEdit(int CategoryId)
        {
            var service = new WISService();
            var inven = HttpContext.Session["ItemInventory"] as List<ItemInventoryModel>;
            var sublst = service.GetSubItemInventoryForEdit(CategoryId);
            var lst = sublst.Where(p => p.ItemInventoryQuantity > 0).Select(p => new SelectListItem { Text = p.ItemInventoryDescription, Value = p.ItemInventoryID.ToString() }).ToList();
            HttpContext.Session["ItemInventory"] = sublst;
            return Json(lst, JsonRequestBehavior.AllowGet);
        }
        ...
        public void ArchiveMonthInvoices(int[] ArchInvoices)
        {
            ...
            foreach (var item in ArchInvoices)
            {
                ...
                else if(InvLst.InvoiceDate.Month==12)
                {
                    var res = service.BulkUpdateInvoiceStatus(item);
                }
            }           
        }
        public void ArchiveEstimates(int[] ArchEstimates)
        {
            var service = new WISService();
            var model = new EstimateModel();
            foreach (var item in ArchEstimates)
            {
                DateTime dateTime = DateTime.UtcNow.Date;
                var InvLst = service.GetEstimateDetails(item);               
                var res = service.BulkUpdateEstimateStatus(item);               
            }
        }
        public void ArchiveMonthGLInvoices(int[] ArchGL)
        {
            var service = new WISService();
            var model = new InvoiceModel();
            foreach (var item in ArchGL)
            {               
                var InvLst = service.GetInvoiceDetails(item);
                var res = service.BulkUpdateGlInvoiceStatus(item);
            }
        }
        public ActionResult ArchivedGlInvoices()
        {
            var service = new WISService();
            List<InvoiceModel> Iim = new List<InvoiceModel>();
            var model = service.GetArchivedGlInvoiceList();
            foreach (var item in model)
            {
                item.CreatedUser = GetUserFullname(item.CreatedUser);
            }
        ...
        public ActionResult ArchivedGlSummary()
        {
            var service = new WISService();
            List<InvoiceModel> Iim = new List<InvoiceModel>();
            var model = service.getInvoiceGLdata();
        ...
        public ActionResult MonthGLInvSummary(int InvoiceMonth,int Invoiceyear)
        {
            var service = new WISService();
            List<InvoiceModel> Iim = new List<InvoiceModel>();
            var model = service.GetGLInvoiceSummaryByMonth(InvoiceMonth,Invoiceyear);
            ViewBag.GLCBTotal = service.GetGLCodeMonthInvoiceSummary("CB29200.000.000030", InvoiceMonth, Invoiceyear).Sum(x => x.InvoiceDetailLineItemTotal);
            ViewBag.GLCWTotal = service.GetGLCodeMonthInvoiceSummary("CW29200.000.000070", InvoiceMonth, Invoiceyear).Sum(x => x.InvoiceDetailLineItemTotal);
            ViewBag.GL2Total = service.GetGLCodeMonthInvoiceSummary("BM29200.000.000050", InvoiceMonth, Invoiceyear).Sum(x => x.InvoiceDetailLineItemTotal);
            //ViewBag.GL1Total = t1;
            foreach (var item in model)
            {
               
            }           
            return View("MonthGLInvSummary",model);
        }
        public ActionResult GLMth(int Month,int Year,String GL)
        {
            var service = new WISService();
            List<InvoiceModel> Iim = new List<InvoiceModel>();
            var model = service.GetGLCodeMonthInvoiceSummary(GL,Month,Year);
            foreach (var item in model)
            {
               
            }
            ViewBag.GLDeatils = GL+" "+ getFullName(Month) + " "+Year;
            return View(model);
        }
        public ActionResult GetInvoiceMonthName()
        {
            var service = new WISService();
            var model = service.GetArchivedGlInvoiceList();
            var monthlst = model.OrderBy(x => x.InvoiceDate);
            return View(monthlst);            
        }
        ...
        public void InvMovetoComplete(int[] MoveInvoices)
        {
            var service = new WISService();
            var model = new InvoiceModel();
            foreach (var item in MoveInvoices)
            {               
                var InvLst = service.GetInvoiceDetails(item);                
                var res = service.UpdateInvoiceStatustoComplete(item);                
                service.updateInvoiceDetail(InvLst);
            }
        }
        public void DeleteInvs(int[] DelInv)
        {
            var service = new WISService();
            var model = new InvoiceModel();
            foreach (var item in DelInv)
            {
                DateTime dateTime = DateTime.UtcNow.Date;
                //var InvLst = service.GetInvoiceDetails(item);
                var res = service.UpdateDeleteInvoiceStatus(item);
            }
        }
        public ActionResult DeleteInvoice()
        {
            var service = new WISService();
            List<InvoiceModel> Iim = new List<InvoiceModel>();
            var model = service.GetDeletedInvoiceList();
            return View(model);
        }
        public ActionResult ArchivedInvoices()
        {
            ...
            {
                item.CreatedUser = GetUserFullname(item.CreatedUser);
                //Invoice is not saving the employeeid ,that causes error for this code
                //if (item.ClientTypeID.Value == 2)
                //{
                //    item.ClientName = service.GetEmployeeNameById(item.employID);
                //}
                if (item.ClientTypeID.Value == 3)
                {
                    item.ClientName = service.GetclientFullname(item.ExternalClientID.Value);
                }
            }
            
            return View(model);
        }
        public ActionResult ArchivedEstimates()
        {
            var service = new WISService();
            List<EstimateModel> Iim = new List<EstimateModel>();
            var model = service.GetArchivedEstimatesList();
            foreach (var item in model)
            {
                item.CreatedUser = GetUserFullname(item.CreatedUser);
            }
        ...
        public ActionResult GetGL1CodeItemInvNumber(string Gl,int catId)
        {           
            var service = new WISService();
            List<InvoiceModel> GlLst = new List<InvoiceModel>();
            var model = service.GetGL1Inventory(Gl,catId);
            return Json(model, JsonRequestBehavior.AllowGet);
        }
        public ActionResult SaveShipmentStatus(string pikup, string del,int ID)
        {
            var service = new WISService();
            var st = service.SaveInvShipmentDtls(pikup, del, ID);
            return View();
        }
        public void UpdateRowInfo(EstimateDetailModel model, int Id, int fromPosition, int toPosition)
        {
            var service = new WISService();
            var st = service.SaveRowOrder(model,Id,fromPosition,toPosition);
        }
        public void UpdateInvoiceRowInfo(InvoiceDetailModel model, int Id, int fromPosition, int toPosition)
        {
            var service = new WISService();
            var st = service.SaveInvoiceRowOrder(model, Id, fromPosition, toPosition);
        }
    }
}
